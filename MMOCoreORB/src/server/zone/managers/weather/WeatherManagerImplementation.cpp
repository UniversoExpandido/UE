...
void WeatherManagerImplementation::sendWeatherTo(CreatureObject* player) {
	...
	if(hasDamagingSandstorms && player->getCurrentWeather() == WeatherManager::EXTREMESTORM) {
		player->sendSystemMessage("La tormenta ha disminuido.");
	}

	player->setCurrentWeather(currentWeather);

	if(hasDamagingSandstorms && currentWeather == WeatherManager::EXTREMESTORM) {
		player->sendSystemMessage("El clima a tu alrededor se ha vuelto peligroso, busca refugio de inmediato!");
	}
...
void WeatherManagerImplementation::applySandstormDamage(CreatureObject* player) {
	...
	if ((!player->isRidingMount() && player->getParentID() != 0) || player->getCurrentCamp() != nullptr) {
		...
		//Blind player
		ManagedReference<SceneObject*> goggles = player->getSlottedObject("eyes");
		 if (goggles != nullptr) {
		//		player->sendSystemMessage("El fuerte viento viene cargado de molestas particulas pero tu gafas te protegen.");
		 } else {
	...
	if (applicableKDChance > 0 && !player->isProne() && !player->isKnockedDown() && System::random(200) > 198 ) {
		if (player->isRidingMount()) {
			player->sendSystemMessage("Una rafaga de viento te tira del vehiculo!");
			player->dismount();
		} else
			player->sendSystemMessage("Una rafaga de viento te desequilibra!");
