...
bool PlayerCreationManager::createCharacter(ClientCreateCharacterCallback* callback) const {
	...
	if (client->getCharacterCount(zoneServer.get()->getGalaxyID()) >= 10) {
		ErrorMessage* errMsg = new ErrorMessage("Create Error", "Estas limitado a 6 personajes por Galaxia.", 0x0);
	...
	if (ghost != nullptr) {
		...
		/* ELIMINAR ESTA LINEA PARA ACTIVAR EL VUELO DESDE LA CREACION DEL PERSONAJE. SE ANYADIRA UNA NAVE EN SU DATAPAD

				/// mejor nave //
							Reference<ShipControlDevice*> shipControlDevice = zoneServer->createObject(STRING_HASHCODE("object/intangible/ship/tieoppressor_pcd.iff"), 1).castTo<ShipControlDevice*>();
							Reference<ShipObject*> ship = zoneServer->createObject(STRING_HASHCODE("object/ship/player/player_tieoppressor.iff"), 1).castTo<ShipObject*>();
				...
				if (accountPermissionLevel < 9) {
					try {
						...
						if (res != nullptr && res->next()) {
							...
							if (timeVal.miliDifference() < 360000) { // 10 munutos eesde la ultima creacion
					...
					if (lastCreatedCharacter.containsKey(accID)) {
						...
						if (lastCreatedTime.miliDifference() < 360000) {  // 10 munutos desde la ultima creacion
							ErrorMessage* errMsg = new ErrorMessage("Create Error", "olo se permite crear un personaje cada 10 minutos. Repetir el intento antes de que pase el tiempo reseteara el contador.", 0x0);
							client->sendMessage(errMsg);
