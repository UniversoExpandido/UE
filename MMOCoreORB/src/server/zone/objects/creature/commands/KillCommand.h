...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		if (targetObject != nullptr) {
			...
			if (targetObject->isPlayerCreature() || targetObject->isPet()) { /// Español: para no usarse en jugadores o pets
				creature->sendSystemMessage("Error. Use /killplayer para matar a un jugador o una mascota.");
